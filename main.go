...
func main() {
	...
	cliConf.createClient("192.168.0.33", 22, "root", "guojinyong")
	//本地文件上传到服务器
	cliConf.Upload(`/tmp/guojinyong/upload.go`, `/root/upload.go`) // /root/upload.go
	//从服务器中下载文件
	cliConf.Download(`/root/test.go`, `/tmp/guojinyong/test.go`) //download succ
